...
const journeyNodes = [{ id: 'top', label: 'START' }, { id: 'about', label: 'DATABASES' }, { id: 'work', label: 'DEVELOPMENT' }, { id: 'xtage', label: 'PRODUCT' }, { id: 'production', label: 'UI / DELIVERY' }, { id: 'lab', label: 'EXPLORATION' }, { id: 'contact', label: 'NOW' }];
function Journey() {
...
export default function Portfolio() {
 ...
 <motion.h1 initial={false} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduced ? 0 : .7 }}>Product & UI<br /><span>Designer<span className="orange">.</span></span></motion.h1>
 <div className="hero-diagram" aria-hidden="true"><svg viewBox="0 0 300 210"><path d="M15 180H70Q90 180 90 160V125Q90 100 115 100H235Q260 100 260 75V35" fill="none" stroke="#444" strokeWidth="1.3"/><path d="M15 180H70Q90 180 90 160V125Q90 100 115 100H190" fill="none" stroke="#ff5200" strokeWidth="2"/><circle cx="15" cy="180" r="4" fill="#ff5200"/><circle cx="120" cy="100" r="4" fill="#ff5200"/><circle cx="190" cy="100" r="5" fill="#ff5200"/><circle cx="260" cy="35" r="4" fill="#050505" stroke="#777"/><text x="14" y="202">DATABASES</text><text x="97" y="80">DEVELOPMENT</text><text x="191" y="80">PRODUCT</text><text x="219" y="18">WHAT’S NEXT?</text></svg><span className="diagram-caption">[ A CONTINUOUS EVOLUTION ]</span></div>
 ...
 <div className="hero-bottom"><div className="hero-stack mono">PRODUCT DESIGN <b>·</b> UI DESIGN <b>·</b> FRONT-END <b>·</b> REACT <b>·</b> NEXT.JS</div><a href="#about" className="scroll mono">{t.scroll}<ArrowDown size={14}/></a></div>
 </section>
 ...
 <section className="section wrap toolkit" id="stack"><div className="section-index mono"><span>06 / {lang === 'pt' ? 'FERRAMENTAS & PRÁTICA' : 'TOOLS & PRACTICE'}</span><Plus size={16}/></div><div className="toolkit-grid"><h2>{t.toolkit}</h2><div className="stack-list">{stack.map((s, i) => <div key={s.name}><h3 className="mono"><span>0{i + 1}</span>{s.name}</h3><Tags items={s.items}/></div>)}</div></div><div className="education-grid"><div><span className="mono muted">{t.educationLabel}</span><h3>{t.education}</h3><p>Descomplica Faculdade Digital</p><span className="mono muted">{t.expected}</span></div><div><span className="mono muted">{t.certLabel}</span><h3>{t.cert}</h3><span className="mono muted">2025</span></div></div></section>
